...
echo "Converting INS Header file from BED to VCF format..."
...
for((eachIndex=0;eachIndex<arrLength;eachIndex++)) do
    ...
    vcfArray+=("$refChr	$qryStart	.	N	<$svType>	.	PASS	SVTYPE=$svType;END=$refEnd;QRYCHR=$refChr;QRYSTART=$refStart;QRYEND=$qryEnd;SVLEN=$svLength	GT:DP	./.:40")
done
