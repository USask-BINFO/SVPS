...
echo "Converting BND Header file from BED to VCF format..."
...
for((eachIndex=0;eachIndex<arrLength;eachIndex++)) do
    ...
    vcfArray+=("$refChr	$refStart	N	N	<BND>	.	PASS	SVTYPE=BND;TRANSSTART=$refStart;TRANSEND=$refEND;	GT:DP	./.:.")
	vcfArray+=("$refChr	$refEnd	N	N	<BND>	.	PASS	SVTYPE=BND;TRANSSTART=$refStart;TRANSEND=$refEND;	GT:DP	./.:.")
done
