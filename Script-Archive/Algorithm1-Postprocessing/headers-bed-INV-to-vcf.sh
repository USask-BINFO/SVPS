...
echo "Converting INV Header file from BED to VCF format..."
...
for((eachIndex=0;eachIndex<arrLength;eachIndex++)) do
	...
	svLength=$((refEnd-refStart))
	svType="INV"

    vcfArray+=("$refChr	$refStart	.	N	<$svType>	.	PASS	SVTYPE=$svType;END=$refEnd;SVLEN=$svLength	GT:DP	./.:40")
done
