...
echo "Converting $INPUT_FILE BED intervals to VCF format for $SV_TYPE..."
...
for((EACH_INDEX=0;EACH_INDEX<ARR_LEN;EACH_INDEX++)) do
	...
	#SV_TYPE="DEL:TRANS"
        ...
        #BND_COUNT=$((BND_COUNT+1))
	VCF_ARR+=("$REF_CHR	$REF_START	${SV_TYPE}.${BND_COUNT}	N	N	.	PASS	SVTYPE=$SV_TYPE;END=$REF_END;TRANSSTART=$REF_END;SVLEN=$SV_LEN	GT:DP	./.:.")
	BND_COUNT=$((BND_COUNT+1))
done
