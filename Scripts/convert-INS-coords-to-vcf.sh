...
for((EACH_INDEX=0;EACH_INDEX<ARR_LEN;EACH_INDEX++)) do
	...
	VCF_ARR+=("$REF_CHR	$REF_START	${SV_TYPE}.${INS_COUNT}	<INS>	$ALT_SEQ	.	PASS	SVTYPE=$SV_TYPE;END=$REF_END;QRYCHR=$REF_CHR;QRYSTART=$REF_START;QRYEND=$QRY_END;SVLEN=$SV_LEN	GT:DP	./.:40")
